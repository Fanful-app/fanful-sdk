...
export default class AuthV1 {
  private static web: { network: AxiosInstance }

  constructor(web: typeof AuthV1.web) {
    AuthV1.web = web
  }
  ...
  public signIn = async (payload: SignInUserInterface): Promise<UserSessionInterface> => {
    const { data } = await AuthV1.web.network.post<BasicResponseInterface<UserSessionInterface>>(
      URLS.signInUser,
      payload
    )
  ...
  public resendOTP = async (payload: SignInUserInterface): Promise<UserInterface> => {
    const { data } = await AuthV1.web.network.post<BasicResponseInterface<UserInterface>>(
      URLS.resendOTP,
      payload
    )
  ...
  /**
   * @method logout
   * @returns {Promise<null>} Logs out a user
   */
  public logout = async (): Promise<null> => {
    const { data } = await AuthV1.web.network.post<BasicResponseInterface>(URLS.logoutUser)
